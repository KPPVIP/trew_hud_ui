...
AddEventHandler('trew_hud_ui:setInfo', function(info)
	...
	SendNUIMessage({ action = 'setMoney', id = 'weight', value = currentWeight })
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0,137) then
			...
			SendNUIMessage({ action = 'element', task = 'enable', value = 'weight' })
			Citizen.Wait(30000)
			mostrando = false
			...
			SendNUIMessage({ action = 'element', task = 'disable', value = 'weight' })
		end	
	end
end)
